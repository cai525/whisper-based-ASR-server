...
class WhisperModel(AbstractWhisperModel):
    ...
    def load(self,
        ...
        self.whisper_model = whisper.load_model(model_name, device, download_root)
    ...
    def transcribe(
        ...
        for seg in (speech_array_indices
                    if len(speech_array_indices) == 1 else tqdm(speech_array_indices)):
            r = self.whisper_model.transcribe(
                audio[int(seg["start"]):int(seg["end"])],
                task="transcribe",
                language=lang,
...
class OpenAIModel(AbstractWhisperModel):
    ...
    def transcribe(
        ...
        audios: List[TypedDict("AudioInfo", {
            "input": str,
            "audio": AudioSegment,
            "start_ms": float
        })] = []
        ...
        for index in speech_array_indices:
            ...
            if len(audio_seg.raw_data) < self.split_audio_bytes:
                ...
                audios.append({"input": temp_file, "audio": audio_seg, "start_ms": start})
            else:
                logging.info(
                ...
                for j in range(split_num):
                    ...
                    split_audio = audio_seg[j * self.split_audio_bytes // ms_bytes:(j + 1) *
                                            self.split_audio_bytes // ms_bytes]
                    audios.append({
                        "input": temp_file,
                        "audio": split_audio,
    ...
    def _transcribe(self, input: srt, audio: AudioSegment, prompt: str, lang: LANG,
                    start_ms: float):
        audio.export(input, "wav")
        subtitles = self.whisper_model(file=open(input, "rb"),
                                       prompt=prompt,
                                       language=lang,
                                       response_format="srt")
        os.remove(input)
        return list(
            map(
                lambda x: (
                    setattr(x, "start", x.start + datetime.timedelta(milliseconds=start_ms)),
                    setattr(x, "end", x.end + datetime.timedelta(milliseconds=start_ms)),
                    x,
                )[-1],
